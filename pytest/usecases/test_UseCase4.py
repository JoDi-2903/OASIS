...
def test_watch_provider_to_id_2():
    watch_provider_codes = TMDB.watch_provider_to_id(
        ["Netflix", "Disney Plus"])
...
def test_watch_provider_to_id_5():
    watch_provider_codes = TMDB.watch_provider_to_id(
        ["Apple TV Plus", "Amazon Prime Video", "Paramount Plus", "Disney Plus", "Netflix"])
...
def test_recommend_random_movie_title():
    random_movie = TMDB.recommend_random_movie(os.getenv('TMDB_API_KEY'), 28)
    assert random_movie['title'] != ""


def test_recommend_random_movie_release_date():
    random_movie = TMDB.recommend_random_movie(os.getenv('TMDB_API_KEY'), 12)
    assert validators.between(
        int(random_movie['release_date'][:4]), min=1750, max=2050)


def test_recommend_random_movie_vote_average():
    random_movie = TMDB.recommend_random_movie(os.getenv('TMDB_API_KEY'), 16)
    assert validators.between(
        float(random_movie['vote_average']), min=0.0, max=10.0)


def test_recommend_random_movie_vote_count():
    random_movie = TMDB.recommend_random_movie(os.getenv('TMDB_API_KEY'), 35)
    assert validators.between(int(random_movie['vote_count']), min=0)


def test_recommend_random_movie_overview():
    random_movie = TMDB.recommend_random_movie(os.getenv('TMDB_API_KEY'), 80)
...
def test_recommend_random_movie_title_N():
    random_movie = TMDB.recommend_random_movie_by_watch_provider(
        os.getenv('TMDB_API_KEY'), 28, "Netflix")
    assert random_movie['title'] != ""


def test_recommend_random_movie_title_A():
    random_movie = TMDB.recommend_random_movie_by_watch_provider(
        os.getenv('TMDB_API_KEY'), 12, "Apple TV Plus")
    assert random_movie['title'] != ""


def test_recommend_random_movie_title_NDA():
    random_movie = TMDB.recommend_random_movie_by_watch_provider(
        os.getenv('TMDB_API_KEY'), 99, "Netflix, Disney Plus, Amazon Prime Video")
    assert random_movie['title'] != ""
